...
export default function SingaporeMobileStickyBar() {
  const trackChannel = () => {
    if (typeof window !== 'undefined') (window as any).gtag?.('event', 'click_mobile_sticky_channel_sg');
  };
  const trackAdmin = () => {
    if (typeof window !== 'undefined') (window as any).gtag?.('event', 'click_mobile_sticky_admin_sg');
  };
