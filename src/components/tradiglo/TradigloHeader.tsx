...
export default function TradigloHeader() {
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          {/* Logo */}
          <a href="/tradiglo.malaysia" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[9px] text-violet-400/60 tracking-[0.25em] uppercase font-semibold">Malaysia</span>
            </div>
          </a>
          ...
          <div className="hidden md:flex items-center gap-3">
            <a
              ...
              onClick={() => typeof window !== 'undefined' && (window as any).gtag?.('event', 'click_whatsapp_admin', { location: 'header' })}
          ...
          <div className="md:hidden flex items-center gap-2">
            ...
            <button
              className="p-2 text-[#A6A8C3] hover:text-white transition-colors rounded-lg hover:bg-white/5"
          ...
          <div className="px-4 py-5 space-y-1">
            {navLinks.map((link) => (
              <button
                key={link.href}
                onClick={() => handleNav(link.href)}
                className="block w-full text-left text-sm text-[#A6A8C3] hover:text-white hover:bg-[rgba(139,92,246,0.08)] px-4 py-3 rounded-lg font-medium transition-all"
              >
                {link.label}
              </button>
            ))}
